...
LOG_FILE = os.path.join(MAIN_OUTPUT_DIR, "prerender_granular_layers_test.log") # Log for this run
logger = setup_simple_logger(LOG_FILE)

logger.info("Starting pre-render script for SEPARATE transparent animated layers (with test frame generation)...")
...
TEST_FRAME_TIME = 5.0  # Time in seconds to generate test frames
...
TEST_FRAMES_OUTPUT_FOLDER = os.path.join(MAIN_OUTPUT_DIR, "test_frames") # Folder for test frames
...
def make_final_coffee_frame(t, coffee_body_img, coffee_blink_img, smoke_images_dict, smoke_anim_params_dict):
    global coffee_next_blink_time_global, coffee_blink_end_time_global # Ensure these are treated as global for updates
...
# --- Frame Generation Function for FINAL NEBBLES Layer ---
def make_final_nebbles_frame(t, zodiac_img, nebbles_body_img, nebbles_eyes_img, video_total_duration):
    current_frame_pil = Image.new('RGBA', CANVAS_SIZE, (0, 0, 0, 0))
    canvas_center_x = CANVAS_SIZE[0] // 2 # Define for this function scope
    canvas_center_y = CANVAS_SIZE[1] // 2 # Define for this function scope
    ...
    if nebbles_body_img:
        nb_target_center_x = canvas_center_x + ZODIAC_PLACEMENT_OFFSET_X 
        nb_target_center_y = canvas_center_y + ZODIAC_PLACEMENT_OFFSET_Y
        nb_paste_x = int(round(nb_target_center_x - nebbles_body_img.width / 2))
        nb_paste_y = int(round(nb_target_center_y - nebbles_body_img.height / 2))
        ...
        if nebbles_eyes_img:
            ...
            final_eye_paste_x = nb_paste_x + eye_anim_offset_x
            final_eye_paste_y = nb_paste_y + eye_anim_offset_y
...
def render_test_frames(time_t, coffee_assets, nebbles_assets, video_duration_for_nebbles_eyes):
    global coffee_next_blink_time_global, coffee_blink_end_time_global # Allow modification for test
    
    logger.info(f"Rendering test frames at t={time_t:.2f}s...")

    # Reset blink state for consistent test frame if called multiple times (though usually called once)
    coffee_next_blink_time_global = time_t + random.uniform(COFFEE_BLINK_MIN_INTERVAL / 2, COFFEE_BLINK_MAX_INTERVAL / 2) 
    coffee_blink_end_time_global = 0 # Ensure blink can happen if t is early
    ...
    test_nebbles_frame = make_final_nebbles_frame(time_t,
                                                  ...
                                                  nebbles_assets["body"],
                                                  nebbles_assets["eyes"],
                                                  video_duration_for_nebbles_eyes) # Pass total duration for eye logic
...
if __name__ == "__main__":
    ...
    # Prepare asset dictionaries for test frame function
    ...
    # Render test frames first
    render_test_frames(TEST_FRAME_TIME, coffee_assets_for_test, nebbles_assets_for_test, TOTAL_DURATION)

    # Confirmation for full render
    total_png_frames = int(TOTAL_DURATION * FPS) * 2 # For two sequences
    ...
    if confirm.lower() == 'yes':
        ...
        # --- Render FINAL COFFEE Animation Sequence ---
        ...
        # --- Render FINAL NEBBLES Animation Sequence ---
